...
"""
Line detection app, the fully-featured version for experiments:
    ...
    - patch-based detection (fixed-size or not) or region-wide detection

For a lighter, more easily maintainable version fit for FSDB context, see its companion app `ddp_line_detect.py`.
The heavy lifting is done by the model defined in the Mask-RCNN module `ddp_lineseg_train.py`.
...
p = {
        ...
        "charter_dirs": [],
        "region_classes": ([], "Names of the layout-app regions on which lines are to be detected. Eg. '[Wr:OldText']. If empty (default), detection is run on the entire page."),
        ...
        "output_format": FargvChoice(["json", "xml", "npy", "stdout"], description="Segmentation output: json=<JSON file>, xml=<PageXML file>, npy=label map (HW), stdout=JSON on standard output."),
        ...
        if len(args.region_classes):
            ...
            with open(layout_file_path, 'r') as regseg_if:
                ...
                if not layout_data:
                    ...
                    continue
                crops_pil, boxes, _ = seglib.layout_regseg_to_crops( img, regseg, args.region_classes, force_rgb=True )
